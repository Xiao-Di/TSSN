...
class FCFineTuner:
    def __init__(self, fc: FC, num_classes: int, num_features: int,
                 in_dim: int, out_dim: int,
                 n_hidden_1: int, n_hidden_2: int, layers: int,
        ...
        self.model = FC(in_dim, n_hidden_1, n_hidden_2, out_dim).to(self.device)
        ...
        self.writer = SummaryWriter('../runs/FC-classify-288')
    ...
    def train(self, epoch):
        ...
        for i, data in data_iter:
            ...
            # 将数据输入分类模型得到分类结果
            classification = self.model(data['bert_input'].reshape(-1,self.in_dim).float())
    ...
    def test(self, data_loader):
        with torch.no_grad():
            ...
            for data in data_loader:
                ...
                result = self.model(data['bert_input'].reshape(-1,self.in_dim).float())
    ...
    def save(self, epoch, file_path):
        output_path = file_path + "fc_classification_checkpoint.tar"
        torch.save({
            'epoch': epoch,
    ...
    def load(self, file_path):
        input_path = file_path + "fc_classification_checkpoint.tar"
