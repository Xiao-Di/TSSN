...
from LSTM_classification_model import LSTM
...
class LSTMFineTuner:
    def __init__(self, lstm: LSTM, num_classes: int, num_features: int,
                 hidden: int, layers: int, dropout,
        ...
        self.model = LSTM(num_features, hidden, layers, num_classes, dropout).to(self.device)
        ...
        self.writer = SummaryWriter('../runs/lstm-288')
    ...
    def train(self, epoch):
        ...
        for i, data in data_iter:
            ...
            # print(data['bert_input'].shape) # [bs, seq_len, num_features]

            # 将数据输入分类模型得到分类结果
            classification = self.model(data['bert_input'].float()) # [bs, 2]
            # print(classification)
            # print(data['class_label'])

            loss = self.criterion(classification, data['class_label'].squeeze().long())
            # print('\nclassify: ', classification)
            # print('label: ', data['class_label'])
    ...
    def test(self, data_loader):
        with torch.no_grad():
            ...
            # 多维list转为一维
            print(classification_result_lists[0])
            print(classification_target_lists)
    ...
    def save(self, epoch, file_path):
        output_path = file_path + "lstm_classification_checkpoint.tar"
        torch.save({
            'epoch': epoch,
    ...
    def load(self, file_path):
        input_path = file_path + "lstm_classification_checkpoint.tar"
