...
class TransformerComplementator:
    def __init__(self, tvtsbert: TVTSBERT, num_features, seq_len, word_len, mask_len,
        ...
        writer_path = '../runs/finetune/transformer_complementation_%d_%d-0104' % (self.word_len, self.mask_len)
    ...
    def train(self, epoch):
        ...
        for i, data in data_iter:
            ...
            finetune_complementation = self.model(data['bert_input'].float(),
                                                  data['bert_mask'].long())  # (num_words,1)
            ...
            loss = self.criterion(finetune_complementation,
                                  data['bert_target48'].float())  # nn.MSELoss(reduction='none'), (288,1)
            ...
            mask = data['loss_mask'].unsqueeze(-1)  # (288,) -> (288,1) => (12,) -> (12,1)
    ...
    def _validate(self):
        with torch.no_grad():
            ...
            for data in self.valid_dataloader:
                ...
                loss = self.criterion(finetune_complementation, data['bert_target48'].float())
    ...
    def test(self, data_loader):
        ...
        with torch.no_grad():
            ...
            print('Saving to transformer_complement_C%d.csv' % (self.mask_len))
            df_save.to_csv(f'/Users/gengyunxin/Documents/项目/traffic_model/TVTS_9.28/finetune/test_result_all/complement_plot_1227/transformer_complement_C{self.mask_len}_i216.csv')
            df_save.to_excel(f'/Users/gengyunxin/Documents/项目/traffic_model/TVTS_9.28/finetune/test_result_all/complement_plot_1227/complement_plot_excel/transformer_complement_C{self.mask_len}_i216.xlsx')

        self.model.train()
        return test_error

    def save(self, epoch, file_path):
        output_path = file_path + "transformer_complement_checkpoint.tar"
        torch.save({
            'epoch': epoch,
    ...
    def load(self, file_path):
        input_path = file_path + "transformer_complement_checkpoint.tar"
