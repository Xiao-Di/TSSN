...
class TransformerPredictor:
    def __init__(self, tvtsbert: TVTSBERT, num_features, seq_len, prediction_len, word_len,
        ...
        self.model = TVTSBERTFTPrediction(tvtsbert, num_features, seq_len, prediction_len, word_len).to(self.device)
        ...
        writer_path = f'../runs/finetune/transformer_prediction_P{prediction_len}_step36'
        ...
        with torch.no_grad():
            ...
            for data in data_loader:
                ...
                prediction_result = self.model(data['bert_input'].float(),
                                               data['bert_mask'].long()) # 1
                prediction_target = data['bert_target'].float() # 1
                input300 = data['bert_input300'].float() # 289
                ...
                input300_inverse = ((max-min)*input300 + min).squeeze()
            ...
            # # print('input300_list max: ', max(input300_list))
    ...
    def save(self, epoch, file_path):
        output_path = file_path + "transformer_predict_checkpoint.tar"
        torch.save({
            'epoch': epoch,
    ...
    def load(self, file_path):
        input_path = file_path + "transformer_predict_checkpoint.tar"
