...
class PretrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 随机噪声
        ts_masking, mask = self.random_masking(ts_processed)

        output = {"bert_input": ts_masking, # 加噪声的时间序列 (288,1)
                  "bert_target": ts_processed, # 原来未加噪声的时间序列 (288,1)
                  "bert_mask": bert_mask, # 有数据的地方是1（长度ts_length）,其他地方是0（全长seq_len） (48,)
                  "loss_mask": mask, # 只计算加噪声位置的loss (288,),加噪声的位置是1,其余位置是0
    ...
    def random_masking(self, ts):
        ...
        ts_masking = ts.copy()
        mask = np.zeros((self.seq_len,), dtype=int)
        # num_words = self.seq_len // self.word_len
        # mask = np.zeros((num_words,), dtype=int)
...
class DatasetWrapper(object):
    ...
    def get_data_loaders(self):
        dataset = PretrainDataset(self.num_features, self.data_path, self.word_len, self.max_length)
