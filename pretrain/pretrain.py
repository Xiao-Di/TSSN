...
# form early_stopping_pytorch.pytorchtools import Earlystopping
from model.tvts_bert import TVTSBERT
from pretrain.prediction_model import TVTSBERTPrediction

# note1: 每次训练设置一下用哪块gpu
...
class TVTSBERTTrainer:
    def __init__(self, tvtsbert: TVTSBERT, num_features: int, word_len: int,
                 train_dataloader: DataLoader, valid_dataloader: DataLoader,
                 lr: float=1e-4, warmup_epochs: int=4, decay_gamma: float=0.99,
                 with_cuda: bool=True, cuda_devices=None, patience=3,
                 log_freq: int=100, gradient_clipping_value=5.0):

        cuda_condition = torch.cuda.is_available() and with_cuda
        self.device = torch.device("cuda" if cuda_condition else "cpu")

        self.tvtsbert = tvtsbert
        self.model = TVTSBERTPrediction(tvtsbert, word_len).to(self.device)
        ...
        # 每次预训练之前改一下writer的地址
        writer_path = '../runs/pretrain_%d' % word_len # note5: tensorboard保存地址
    ...
    def train(self, epoch):
        ...
        for i, data in data_iter:
            ...
            # print('target: ', data['bert_target'].shape)
    ...
    def save(self, epoch, file_path):
        ...
        bert_path = file_path + "checkpoint_epoch%d.bert.pth" % epoch
