...
class InputEmbedding(nn.Module):
    ...
    def __init__(self, word_len, embedding_dim, dropout=0.1):
        """
        ...
        """
        super().__init__()
        # self.input1 = nn.Linear(in_features=word_len, out_features=1)
        self.input = nn.Linear(in_features=word_len, out_features=embedding_dim)
        self.dropout = nn.Dropout(p=dropout)
    ...
    def forward(self, input_sequence):
        ...
        # embed = self.input(input_sequence.view(batch_size, -1, self.word_len))
...
#     '''
    ...
    #
    # def forward(self, x):
    #     print('x size before pe: ', x.shape)
    #     print('pe matrix size: ', self.pe.shape)
    #     print(self.pe[:,:x.size(1)].shape)
    #
    #     print((x * torch.sqrt(self.d_model) + self.pe[:,:x.size(1)]).shape)
...
class PositionalEncoding(nn.Module):
    ...
    output:
        tensor: B x T
    """
    def __init__(self, d_model, pe_window=288, dropout=0.1):
        super(PositionalEncoding, self).__init__()
        self.dropout = nn.Dropout(p=dropout)

        # Compute the positional encodings once in log space.
        pe = torch.zeros(pe_window, d_model)
        position = torch.arange(0., pe_window).unsqueeze(1)
    ...
    def forward(self, x):
        # print('x before pe: ', x.shape)
        x = x + self.pe[:, :x.size(1)]
        # print('pe matrix: ', self.pe.shape)
        # print('x after pe: ', x.shape)
        return self.dropout(x)

# 模型拼接TVTS-BERT
class TVTSBERT(nn.Module):

    def __init__(self, word_len, hidden, n_layers, attn_heads, pe_window, dropout=0.1):
        """
        ...
        self.embedding = InputEmbedding(word_len, int(hidden))

        self.pe = PositionalEncoding(hidden, pe_window)
    ...
    def forward(self, x, mask):
        ...
        # print('x in: ', x.shape)
        x = x.view(batch_size, -1, self.word_len)
        ...
        # print('x after input embedding: ', x.shape)
        x = self.pe(x)
        # print('x after position encoding / x in transformer:', x.shape)
